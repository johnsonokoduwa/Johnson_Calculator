...
def Fjohnson():
        ...
        expression = float(math.sqrt((A*A)+(B*B)))
...
if __name__ == "__main__": 
        ...
        Jt1 = Label (gui, text="Insert A(Green):", font=("Arial Bold", 10),bg="grey")
        Jt1.grid(row =7, column=0)

        textbox2 = Entry(gui, bg='silver', width=7)
        textbox2.grid(row=7, column=3)

        Jt2 = Label (gui, text="Insert B(Silver):", font=("Arial Bold", 10),bg="grey")
        Jt2.grid(row =7, column=2)
        
        buttonCalc = Button(gui, text='Square root of A(G)&B(S) TextBox,√(A^2+B^2)', bg='orange', command=Fjohnson, height=1, width=35)
        buttonCalc.grid(row=7, column=4)

        buttonX = Button(gui, text=' X^2 ', fg='black', bg='blue', command=square, height=1, width=7) 
